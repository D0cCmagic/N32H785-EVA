...
*\*\fun     OTA 升级命令检测（逐字节滚动匹配 "UPDATE"+行终止符）
**/
void USART1_IRQHandler(void)
{
    uint8_t ch;

    if (USART_GetFlagStatus(LOG_USARTx, USART_FLAG_RXDNE) != RESET)
    {
        ch = (uint8_t)USART_ReceiveData(LOG_USARTx);   
			
        /* 滚动匹配 OTA 升级命令，命中后由主循环执行复位进 boot */
        if (ota_cmd_feed(ch))
        {
            ota_request_flag = 1;
						ota_request_reset();
