...
#define HGIC_RAW_MAX_PAYLOAD (1800 - sizeof(struct hgic_frm_hdr2) - sizeof(struct hgic_eth_hdr))
#define HGIC_RAW_DATA_ROOM   (sizeof(hgic.tx_buf) - sizeof(struct hgic_frm_hdr2) - sizeof(struct hgic_eth_hdr))
...
extern int hgic_platform_raw_send(unsigned char *data, unsigned int len);

/* 宏映射: HGIC 驱动里调用 raw_send(), 改名避免和 LWIP raw_send 冲突 */
#define raw_send(data, len)    hgic_platform_raw_send((data), (len))
