...
int hgic_raw_send(unsigned char *dest, unsigned char *data, unsigned int len)
{
    ...
    struct hgic_eth_hdr *ehdr = (struct hgic_eth_hdr *)(hdr + 1);
    ...
    hdr->hdr.length = len + sizeof(struct hgic_frm_hdr2) + sizeof(struct hgic_eth_hdr);
    hdr->hdr.ifidx  = 1;
    ...
    return hgic_platform_raw_send(hgic.tx_buf, len + sizeof(struct hgic_frm_hdr2) + sizeof(struct hgic_eth_hdr));
}
...
int hgic_raw_send_ether(unsigned char *data, unsigned int len)
{
    ...
    return hgic_platform_raw_send(hgic.tx_buf, len + sizeof(struct hgic_frm_hdr2));
}
